...
export async function updateHotel(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const areaEn = formData.get('area_en') as string
    ...
    if (areaCode !== undefined) updateData.area_code = areaCode || null
    if (areaKo !== undefined) updateData.area_ko = areaKo || null
    if (areaEn !== undefined) updateData.area_en = areaEn || null

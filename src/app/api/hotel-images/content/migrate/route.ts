...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('본문 이미지 마이그레이션 요청:', { sabreId, imageCount: images.length })
      ...
      try {
        ...
        if (existingOriginal) {
          ...
          if (imageResponse.ok) {
            ...
            if (existingOriginal.size === newSize) {
              ...
              console.log(`호텔 ${sabreId} 본문 이미지 ${nextSeq} 스킵: 동일한 크기`)
            }
          }
        }
        ...
        if (shouldUpload) {
          ...
          if (publicUploadError) {
            console.warn(`호텔 ${sabreId} 본문 이미지 ${nextSeq} 공개 파일 생성 실패: ${publicUploadError.message}`)
          }
        }
        ...
        errors.push(`이미지 처리 실패 (${imageData.url}): ${imageError instanceof Error ? imageError.message : '알 수 없는 오류'}`)
      }
    }

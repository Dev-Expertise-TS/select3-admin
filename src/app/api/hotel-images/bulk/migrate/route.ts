...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!bucketExists) {
      const { error: createBucketError } = await supabase.storage.createBucket(MEDIA_BUCKET, {
        public: true,
      })
      ...
      if (createBucketError) {
        ...
        return NextResponse.json(
          { success: false, error: `버킷 생성 실패: ${createBucketError.message}` },
          { status: 500 }
        )
      }
    }
    ...
    // 배치별로 호텔 처리
    for (let batchStartId = startId; batchStartId <= maxId; batchStartId += batchSize) {
      const batchEndId = Math.min(batchStartId + batchSize - 1, maxId)
      
      console.log(`배치 ${currentBatch}/${totalBatches} 처리 중: ID ${batchStartId} ~ ${batchEndId}`)

      try {
        // 현재 배치의 호텔들 조회 (이미지가 있는 호텔만)
        const { data: hotels, error: fetchError } = await supabase
          .from('select_hotels')
          .select('id_old, sabre_id, slug, property_name_ko, property_name_en, image_1, image_2, image_3, image_4, image_5')
          .gte('id_old', batchStartId)
          .lte('id_old', batchEndId)
          .not('id_old', 'is', null)
          .or('image_1.not.is.null,image_2.not.is.null,image_3.not.is.null,image_4.not.is.null,image_5.not.is.null')

        if (fetchError) {
          errors.push(`배치 ${currentBatch} 호텔 조회 실패: ${fetchError.message}`)
          continue
        }
        ...
        for (const hotel of hotels) {
          try {
              ...
              try {
                ...
                if (existingOriginal) {
                  ...
                  if (imageResponse.ok) {
                    ...
                    if (existingOriginal.size === newSize) {
                      shouldUpload = false
                      console.log(`호텔 ${hotel.sabre_id} ${column} 스킵: 동일한 크기`)
                    }
                  }
                }
                ...
                if (shouldUpload) {
                  ...
                  if (publicUploadError) {
                    console.warn(`호텔 ${hotel.sabre_id} ${column} 공개 파일 생성 실패: ${publicUploadError.message}`)
                  }
                }
                ...
                hotelSuccess = false
                errors.push(`호텔 ${hotel.sabre_id} 이미지 ${i + 1} 처리 실패: ${imageError instanceof Error ? imageError.message : '알 수 없는 오류'}`)
              }
            }

            if (hotelSuccess && migratedImages.length > 0) {
              statistics.successfulMigrations++
              console.log(`호텔 ${hotel.sabre_id} 마이그레이션 성공: ${migratedImages.length}개 이미지`)
            } else {
            ...
            statistics.failedMigrations++
            errors.push(`호텔 ${hotel.sabre_id} 처리 실패: ${hotelError instanceof Error ? hotelError.message : '알 수 없는 오류'}`)
          }
        }

      } catch (batchError) {
        errors.push(`배치 ${currentBatch} 처리 실패: ${batchError instanceof Error ? batchError.message : '알 수 없는 오류'}`)
      }
      ...
      if (currentBatch % 5 === 0 || currentBatch === totalBatches) {
        console.log(`진행률: ${currentBatch}/${totalBatches} (${((currentBatch / totalBatches) * 100).toFixed(1)}%)`)
      }
    }

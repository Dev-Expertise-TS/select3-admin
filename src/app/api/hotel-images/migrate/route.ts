...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      try {
        ...
        if (existingOriginal) {
          ...
          console.log(`기존 파일 발견: ${originalPath}, 크기: ${existingSize} bytes`)
        }

        // URL에서 이미지 다운로드
        const imageResponse = await fetch(url)
        if (!imageResponse.ok) {
          errors.push(`이미지 다운로드 실패 (${column}): ${imageResponse.status} ${imageResponse.statusText}`)
          continue
        }
        ...
        if (shouldUpload) {
          ...
          if (uploadError) {
            errors.push(`원본 파일 업로드 실패 (${column}): ${uploadError.message}`)
            continue
          }
        ...
        if (shouldUploadPublic) {
          ...
          if (publicUploadError) {
            errors.push(`공개 파일 생성 실패 (${column}): ${publicUploadError.message}`)
          } else {
        ...
        errors.push(`이미지 처리 중 오류 (${images[i]?.column || '알 수 없음'}): ${error instanceof Error ? error.message : '알 수 없는 오류'}`)
      }
    }
    ...
    const publicUploadedCount = migratedFiles.filter(f => f.publicUploaded).length
    const publicSkippedCount = migratedFiles.filter(f => !f.publicUploaded).length
...
export async function GET(request: NextRequest) {
  try {
    ...
    const hotelMediaBucket = buckets?.find(bucket => bucket.name === MEDIA_BUCKET)

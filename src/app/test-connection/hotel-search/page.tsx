...
const apiEndpoints: ApiEndpoint[] = [
  // Dashboard APIs
  {
    ...
    response:
      "{ success: true, data: { totalHotels: 1234, chainCount: 56, ... } }",
  },
  ...
  },
  {
    ...
    category: "Hotel",
    parameters:
      "FormData (sabre_id, property_name_ko, property_name_en, brand_id, etc.)",
  ...
  // Rate Plan APIs
  {
    ...
    category: "Rate Plan",
    parameters:
      "{ sabre_id: string, check_in: string, check_out: string, rate_plan_code?: string[] }",
    example:
      'POST /api/rate-plan-codes\n{ "sabre_id": "123", "check_in": "2024-01-01", "check_out": "2024-01-02" }',
  ...
  },
  {
    ...
    category: "Chain & Brand",
    parameters:
      "{ brand_id: number, name_kr: string, name_en: string, chain_id: number }",
    example:
  ...
  // External APIs (외부 서비스용)
  {
    id: "external-rate-plan-codes",
    method: "GET",
    path: "/api/hotel/rate-plan-codes",
    description:
      "특정 호텔의 Rate Plan Code 목록 조회 (외부 API - Bearer 토큰 필요)",
    category: "External API",
    parameters:
      "Query: sabre_id (required)\nHeader: Authorization: Bearer {API_TOKEN}",
    example:
      "GET /api/hotel/rate-plan-codes?sabre_id=390433\n\nHeaders:\n  Authorization: Bearer YOUR_API_TOKEN",
    response:
      '// 성공 (200)\n{ success: true, data: ["API", "XLO", "PPR"] }\n\n// 인증 실패 (401)\n{ success: false, error: "인증 토큰이 유효하지 않습니다" }\n\n// 파라미터 누락 (400)\n{ success: false, error: "sabre_id 파라미터가 필요합니다" }\n\n// 호텔 없음 (404)\n{ success: false, error: "해당 sabre_id의 호텔을 찾을 수 없습니다" }',
  },
];
...
export default function ApiEndpointTest() {
  ...
  const categories = [
    "All",
    ...Array.from(new Set(apiEndpoints.map((api) => api.category))),
  ];
  ...
  return (
    <div className="min-h-[60vh]">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            프로젝트에서 사용하는 모든 API 엔드포인트 목록과 테스트 도구
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between text-sm text-gray-500">
        <div>
          총 {apiEndpoints.length}개의 API 엔드포인트 ({filteredApis.length}개
          표시)
        </div>
        <div className="flex items-center gap-4">
          {Object.entries(methodColors).map(([method, colorClass]) => (
            <div key={method} className="flex items-center gap-1">
              <span
                className={cn("w-3 h-3 rounded-full border", colorClass)}
              ></span>
              <span>{method}</span>
            </div>
          ))}

...
import { Database, FileText, AlertCircle, CheckCircle, Loader2, Search, Image as ImageIcon } from 'lucide-react'
import { Button } from '@/components/ui/button'
import HotelSearchWidget from '@/components/shared/hotel-search-widget'

interface ContentMigrationStatus {
  status: 'idle' | 'searching' | 'extracting' | 'migrating' | 'completed' | 'error'
  message?: string
  extractedImages?: {
...
export function ContentImageMigrationPanel() {
  ...
  const [migrationStatus, setMigrationStatus] = useState<ContentMigrationStatus>({ status: 'idle' })
  ...
  const handleContentMigration = async () => {
    ...
    setMigrationStatus({ status: 'searching', message: '호텔 정보를 조회하는 중...' })
    ...
    try {
      ...
      setMigrationStatus({ 
        status: 'completed', 
        message: migrateResult.message || '본문 이미지 마이그레이션이 완료되었습니다.',
        extractedImages: extractResult.data.extractedImages,
      ...
      setMigrationStatus({ 
        status: 'error', 
        message: error instanceof Error ? error.message : '본문 이미지 마이그레이션 중 오류가 발생했습니다.' 
      })
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border p-6">
        ...
        {selectedHotel && (
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            ...
            <div className="text-sm text-blue-800">
              ...
              <div><strong>슬러그:</strong> {selectedHotel.slug}</div>
            </div>
          </div>
        )}
        ...
        {selectedHotel && (
          <div className="mt-4">
            <Button
              ...
              disabled={migrationStatus.status === 'searching' || migrationStatus.status === 'extracting' || migrationStatus.status === 'migrating'}
              className="flex items-center gap-2 w-full"
            >
              {migrationStatus.status === 'searching' || migrationStatus.status === 'extracting' || migrationStatus.status === 'migrating' ? (
        ...
        <div className="mt-4 text-sm text-gray-600">
          <p>• <code>property_details</code>와 <code>property_location</code> 컬럼에서 이미지 URL을 추출합니다</p>
      ...
      {/* 추출된 이미지 목록 */}
      {migrationStatus.extractedImages && migrationStatus.extractedImages.length > 0 && (
        <div className="bg-white rounded-lg border p-6">
      ...
      {/* 마이그레이션 결과 */}
      {migrationStatus.migratedImages && migrationStatus.migratedImages.length > 0 && (
        <div className="bg-white rounded-lg border p-6">
          <h3 className="text-lg font-semibold mb-4">마이그레이션 결과</h3>
          <div className="space-y-3">
            {migrationStatus.migratedImages.map((image, index) => (
              <div key={index} className="flex items-start gap-4 p-3 border rounded-lg bg-green-50">
                {/* 마이그레이션된 이미지 미리보기 */}
                <div className="flex-shrink-0">
                  <div className="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden border">
                    <img

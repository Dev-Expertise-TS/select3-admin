...
import { Download, Database, AlertCircle, CheckCircle, Loader2, Search, Image as ImageIcon } from 'lucide-react'
...
export function ImageMigrationPanel() {
  ...
  const [previewPaths, setPreviewPaths] = useState<{ original: string; public: string }[]>([])
  ...
  const fetchHotelImages = async (sabreId: string) => {
    setMigrationStatus({ status: 'fetching', message: '호텔 이미지 정보를 조회하는 중...' })
    ...
    try {
      ...
      if (result.data.hotel?.slug) {
        ...
        setSelectedHotel(prev => prev ? { ...prev, slug: result.data.hotel.slug } : null)
      }
      ...
      setMigrationStatus({ 
        status: 'error', 
        message: error instanceof Error ? error.message : '이미지 조회 중 오류가 발생했습니다.' 
      })
    }
  }
  ...
  const handleMigration = async () => {
    ...
    setMigrationStatus({ status: 'migrating', message: '이미지 마이그레이션 중...', progress: 0 })
    ...
    try {
      ...
      const detailedMessage = result.message || `마이그레이션 완료: 업로드 ${statistics?.uploadedCount || 0}개, 스킵 ${statistics?.skippedCount || 0}개`
  ...
  const handlePathMigration = async () => {
    ...
    setMigrationStatus({ status: 'migrating', message: '이미지 경로 마이그레이션 중...' })
    ...
    try {
      ...
      setMigrationStatus({ 
        status: 'completed', 
        message: result.message || '이미지 경로가 성공적으로 마이그레이션되었습니다.',
        migratedImages: migratedImages
      })
      ...
      setMigrationStatus({ 
        status: 'error', 
        message: error instanceof Error ? error.message : '경로 마이그레이션 중 오류가 발생했습니다.' 
      })
    }
  }
    ...
    <div className="space-y-6">
        ...
        {selectedHotel && (
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            ...
            <div className="text-sm text-blue-800">
              <div><strong>Sabre ID:</strong> {selectedHotel.sabreId}</div>
              <div><strong>한글명:</strong> {selectedHotel.nameKr}</div>
              <div><strong>영문명:</strong> {selectedHotel.nameEn}</div>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-lg border p-6">
          ...
          <div className="space-y-4">
            {hotelImages.map((image) => (
              <div key={image.column} className="flex items-start gap-4 p-4 border rounded-lg">
                <input
                  type="checkbox"
                  ...
                  <div className="w-20 h-20 bg-gray-100 rounded-lg overflow-hidden border">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        target.parentElement!.innerHTML = '<div class="w-full h-full flex items-center justify-center text-gray-400"><span class="text-xs">이미지 로드 실패</span></div>'
                      }}
                    />
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="font-medium text-gray-900">{image.column}</span>
                  </div>
            ...
            {previewPaths.map((path, index) => (
              <div key={index} className="bg-gray-50 p-3 rounded">
                <div className="text-sm">
                  ...
                  <div className="font-medium text-gray-700 mb-1 mt-2">공개:</div>
                  <div className="text-xs font-mono text-gray-600 break-all">
        ...
        <div className="bg-white rounded-lg border p-6">
          <div className="flex gap-4">
            <Button
              ...
              disabled={!hotelSlug || hotelImages.filter(img => img.checked).length === 0 || migrationStatus.status === 'migrating'}
      ...
      {migrationStatus.message && (
        <div className={`p-4 rounded-lg ${
          ...
          {/* 상세 통계 표시 */}
          {migrationStatus.status === 'completed' && migrationStatus.statistics && (
            <div className="mt-3 text-sm">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="font-medium text-gray-700 mb-1">원본 파일</div>
                  <div className="space-y-1">
                    <div className="flex justify-between">
                      <span>업로드:</span>
                      <span className="font-medium text-green-600">{migrationStatus.statistics.uploadedCount}개</span>
                    </div>
                    <div className="flex justify-between">
                      <span>스킵:</span>
                      <span className="font-medium text-yellow-600">{migrationStatus.statistics.skippedCount}개</span>
                    </div>
                  </div>
                </div>
                <div>
                  <div className="font-medium text-gray-700 mb-1">공개 파일</div>
                  <div className="space-y-1">
                    <div className="flex justify-between">
                      <span>업로드:</span>
                      <span className="font-medium text-green-600">{migrationStatus.statistics.publicUploadedCount}개</span>
                    </div>
                    <div className="flex justify-between">
                      <span>스킵:</span>
                      <span className="font-medium text-yellow-600">{migrationStatus.statistics.publicSkippedCount}개</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* 마이그레이션 결과 이미지 */}
      {migrationStatus.status === 'completed' && migrationStatus.migratedImages && migrationStatus.migratedImages.length > 0 && (
        <div className="bg-white rounded-lg border p-6">
          <h3 className="text-lg font-semibold mb-4">마이그레이션 결과 이미지</h3>
          <div className="space-y-4">
            {migrationStatus.migratedImages.map((image) => (
              <div key={image.column} className="flex items-start gap-4 p-4 border rounded-lg bg-green-50">
                {/* 마이그레이션된 이미지 미리보기 */}
                <div className="flex-shrink-0">
                  <div className="w-20 h-20 bg-gray-100 rounded-lg overflow-hidden border">
                    <img
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="space-y-2">
                    <div>
                      <div className="text-xs text-gray-500 mb-1">새로운 Supabase Storage URL:</div>
                    ...
                    {image.originalUrl && (
                      <div>
                        <div className="text-xs text-gray-500 mb-1">기존 URL:</div>
